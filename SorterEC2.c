...
struct csv *parseCSV() {
	...
	ret->numColumns = headerInfo.numColumns;
...
struct headerInfo getHeaderInfo() {
	...
	char **columnNames = malloc(sizeof(char *) * maxColumns);
	enum type *types = malloc(sizeof(enum type) * maxColumns);
	...
	//Scan first row for column names
	while (!newlineFound) {
	...
	//Scan second row for column types.
	while (!newlineFound) {
		currentInput = malloc(sizeof(char) * maxStringSize);
		stringPosition = 0;
		while (scanf("%c", &nextChar) > 0) {
		...
		//Add null-terminating 0 to end of String.
		currentInput = addCharacterToString(currentInput, '\0', stringPosition);
		if (strcmp(currentInput, "string") == 0) {
			types[retPosition] = string;
		} else if (strcmp(currentInput, "integer") == 0) {
			types[retPosition] = integer;
		} else if (strcmp(currentInput, "decimal") == 0) {
			types[retPosition] = decimal;
		} else {
			printf("Invalid data type found when parsing CSV.\n");
			exit(0);
		}
...
struct entryInfo getCSVEntries(enum type *columnTypes) {
	...
	while (!eofReached) {
		...
		currentEntry -> values = malloc(sizeof(union value) * maxColumns);
		...
		while (!newlineFound && !eofReached) {
			...
			if (next == '\r' || next == '\n') {
				...
				if (currentValuePosition > maxColumns) {
					newlineFound = 1;
				}
				stringPosition = 0;	
